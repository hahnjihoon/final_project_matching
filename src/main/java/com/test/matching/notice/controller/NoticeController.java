...
public class NoticeController {
	...
	public String moveUpdatePage(@RequestParam("notice_num") int notice_num, Model model) {
		Notice notice = noticeService.selectNotice(notice_num);
		if(notice != null) {
			model.addAttribute("notice", notice);
			return "notice/noticeUpdateForm";
		}else {
			model.addAttribute("message", notice_num+"번 글 수정페이지이동 실패");
			return "common/error";
		}
	}
	...
	public String noticeNewTop3Method(HttpServletResponse response) throws UnsupportedEncodingException {
		...
		for(Notice notice : list) {
			...
			job.put("notice_num", notice.getNotice_num());
			job.put("notice_title", URLEncoder.encode(notice.getNotice_title(), "utf-8"));
			//한글은 인코딩 디코딩해야된다 여기서인코딩 저기받는뷰?에서 디코딩
			job.put("notice_date", notice.getNotice_date().toString());  //스트링으로안바꾸면null로됨 날짜만
...
//		} //첨부파일있을때만?
	...
	public String noticeDeleteMethod(@RequestParam("notice_num") int notice_num, 
		...
		if(noticeService.deleteNotice(notice_num) > 0) {
			...
			model.addAttribute("message", notice_num + "번 글 삭제실패");
			return "common/error";
		}
...
//		} //새로첨부된파일있다면
		...
		if(noticeService.updateNotice(notice) > 0) {
			...
			model.addAttribute("message", notice.getNotice_num()+" 번 공지 수정실패");
			return "common/error";
		}
	}
